...
func resourceLtsElbCreate(ctx context.Context, d *schema.ResourceData, meta interface{}) diag.Diagnostics {
	...
	LogTank := entity.CreateLogTankOption{
		...
		LogBalancerId: d.Get("logbalancer_id").(string),
		LogTopicId:    d.Get("log_stopic_id").(string),
	}
	LogTankRequest := entity.CreateLogtankRequestBody{
		Logtank: &LogTank,
	}
	client.WidthMethod(httpclient_go.MethodPost).WithUrl(url).WithHeader(header).WithBody(LogTankRequest)
	...
	return diag.Errorf("error creating LogTank fields %s: %s", LogTankdRequest, err)
}
...
func resourceLtsElbRead(_ context.Context, d *schema.ResourceData, meta interface{}) diag.Diagnostics {
	...
	mErr := multierror.Append(nil,
		d.Set("logbalancer_id", rlt.Logtank.LoadBalancerID),
...
func resourceLtsElbUpdate(ctx context.Context, d *schema.ResourceData, meta interface{}) diag.Diagnostics {
	...
	client.WidthMethod(httpclient_go.MethodPut).WithUrl(url).WithHeader(header).WithBody(LogTankRequest)
	response, err := client.Do()
	if err != nil {
